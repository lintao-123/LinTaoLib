...
$(function(){
	...
	$("#dg").datagrid({
		idField:"bookno",
		fit:true,
		url:"BookdetailedServlet?method=findAll",
		fitColumns:true,
		rownumbers:true,
		striped:true,
		loadMsg:"正在加载数据，请稍候....",
		onDblClickRow: function(index,row){
			window.location="salesinquiry.jsp";
		},
		frozenColumns:[[
			{field:"checkbox",checkbox:true}
		]],
		columns:[[
			{field:"bookno",title:"销售编号",align:"center",width:100,hidden:true},
			{field:"ordernumber",title:"销售订单号",align:"center",width:100},
			{field:"bookname",title:"客户名称",align:"center",width:100},
			{field:"bookprice",title:"销售日期",align:"center",width:100},
			{field:"salesman",title:"操作员",align:"center",width:100}
		]],
		//分页工具栏
		pagination:true,
		pageSize:12,
	...
	$("#stockdialog").dialog({
		...
		buttons:[{
			...
			handler:function(){
				...
				$.ajax({
					type:"post",
					url:"BookdetailedServlet?method=addBookdetailed",
					cache:false,
					data:$form.serialize(),
					dataType:"text",
	...
	if(array.length==0){
		...
		$.messager.confirm("确认","是否真的删除?",function(r){
			if(r){
				...
				//post提交
				$.post("BookdetailedServlet?method=deleteBookdetailed",
						{bookdetailedArray:indentStr},
						function(message){
							if(message.indexOf("success")!=-1){
								$.messager.show({
									title:"提示",
	...
	$("#stockdialog").dialog({
		...
		buttons:[{
			...
			handler:function(){//类似于添加
				...
				$.post("BookdetailedServlet?method=updateBookdetailed",
					$form.serialize(),
					function(message){
						if(message.indexOf("success")!=-1){
						$.messager.show({
...
<body  class="easyui-layout">      
	...
	<div data-options="region:'center',title:'销售订单表'" style="padding:5px;background:#eee;">   
    <!-- datagrid -->
	<table id="dg">
	</table>
	<!-- 新增/修改窗口 -->
	<div id="stockdialog" class="easyui-dialog" title="增加" style="width:330px;height:240px;"
		...
		<form id="stockform">
			...
			<table id="stocktable">
				...
				<tr>
					<td>销售日期</td>
					<td>
						<input type="text" name="bookprice" class="easyui-datebox"
						data-options="required:true,missingMessage:'请输入销售日期'"/>
					</td>
				</tr>
				<tr>
					<td>操作员</td>
					<td>
						 <select class="easyui-combobox" name="salesman" style="width:142px;">
